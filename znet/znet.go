...
import (
	...
	"github.com/meta-apex/zenith/znet/internal/gfd"
)
...
var (
	allEngines sync.Map
	// shutdownPollInterval is how often we poll to check whether engine has been shut down during znet.Stop().
...
//
// Note that the parameter znet.Conn might have been already released when it's UDP protocol,
...
func createListeners(addrs []string, opts ...Option) ([]*listener, *Options, error) {
	...
	if options.Logger == nil {
		options.Logger = zlog.GetDefaultLogger().WithName("znet")
	}
	...
	// makes it pointless to enable SO_REUSEPORT on *BSD and Darwin for znet with
