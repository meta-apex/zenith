...
// Options are configurations for the znet application.
type Options struct {
	...
	// then znet will use the default logger powered by go.uber.org/zap.
	Logger *zlog.Logger
	...
	// EdgeTriggeredIOChunk specifies the number of bytes that `znet` can
...
// WithMulticore enables multi-cores mode for znet engine.
...
// WithLoadBalancing picks the load-balancing algorithm for znet engine.
...
// WithNumEventLoop sets the number of event loops for znet engine.
...
// WithEdgeTriggeredIOChunk sets the number of bytes that `znet` can
