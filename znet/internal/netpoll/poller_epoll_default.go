...
func (p *Poller) Polling(callback PollEventHandler) error {
	...
	for {
		...
		if n == 0 || (n < 0 && err == unix.EINTR) {
			...
			zlog.Error().Msgf("error occurs in epoll: %v", os.NewSyscallError("epoll_wait", err))
			return err
		}
		msec = 0
		...
		if doChores {
			...
			if (!p.asyncTaskQueue.IsEmpty() || !p.urgentAsyncTaskQueue.IsEmpty()) && atomic.CompareAndSwapInt32(&p.wakeupCall, 0, 1) {
				for {
					...
					if err != nil {
						zlog.Error().Msgf("failed to notify next round of event-loop for leftover tasks, %v", os.NewSyscallError("write", err))
					}
					break
				}
			}
