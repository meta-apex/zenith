...
func (rl *RedisLock) AcquireCtx(ctx context.Context) (bool, error) {
	...
	if errors.Is(err, red.Nil) {
		...
		zlog.Error().Msgf("Error on acquiring lock for %s, %s", rl.key, err.Error())
		return false, err
	} else if resp == nil {
		return false, nil
	}
	...
	zlog.Error().Msgf("Unknown reply when acquiring lock for %s: %v", rl.key, resp)
	return false, nil
}
