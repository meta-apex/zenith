...
var (
	...
	slowThreshold = zsync.ForAtomicDuration(defaultSlowThreshold)
)
...
func NewRedis(conf RedisConf, opts ...Option) (*Redis, error) {
	...
	if !conf.NonBlock {
		if err := rds.checkConnection(conf.PingTimeout); err != nil {
			return nil, zerror.Wrap(err, fmt.Sprintf("redis connect error, addr: %s", conf.Host))
		}
	}
...
func acceptable(err error) bool {
	return err == nil || zerror.In(err, red.Nil, context.Canceled)
}
